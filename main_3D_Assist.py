...
from reid.datasets.domain_adaptation import DA , DA_PersonX

from reid import models
from reid.trainers import Trainer
from reid.evaluators import Evaluator
from reid.utils.data import transforms as T
from reid.utils.data.preprocessor import Preprocessor, UnsupervisedCamStylePreprocessor, PersonX_Preprocessor
...
def get_data(data_dir, source, target, height, width, batch_size, re=0, workers=8):
    ...
    if source == 'PersonX':
        source_list_dir = osp.join(data_dir, 'PersonX', 'data_list')
        dataset = DA_PersonX(data_dir, source_list_dir, target)
    else:
    ...
    source_train_loader = DataLoader(
        PersonX_Preprocessor(dataset.source_train, transform=train_transformer),
...
def main(args):
    ...
    if not args.evaluate:
        sys.stdout = Logger(osp.join(args.logs_dir, 'log_%s_%s_C.txt' % (args.source, args.target)))
...
if __name__ == '__main__':
    ...
    # source
    parser.add_argument('-s', '--source', type=str, default='PersonX',
                        choices=['market', 'duke', 'msmt17', 'PersonX'])
    # target
